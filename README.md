...
The analysis phase starts "consuming" each token from the collection and ends when no token is left (or when it reaches an `END` token which could be manually appended at the end of the lexical analysis phase). The list of rules mentioned in the above paragraph defines all valid sequences of tokens. There are strict rules for how an `if` statement or an assignment operation needs to look like.

One may start to check different rules on their inputs. For example, a simplified variant of the `if` rule requires that after an `IF` token, an `LPAR` needs to follow up. Consuming something else after the `IF` token will result in a compilation error, as a specific token was expected, but not received. In order to implement the rules we will verify the sequence of tokens in nested `if` statements:

```
ruleIf:
  if token IF is consumed
    if token LPAR is consumed
      if expression is consumed
        if token RPAR is consumed
          ...
```

`expression` is called "non-terminal", as it isn't a singular token, but another list of checks for a separate rule (that can be included in other rules, if required).

After every verification (instead of the first one), an error should be thrown. For example, if, after the expression, end of file (or another token different from `RPAR`) is reached, it means that the syntax of the `IF` statement is not valid, because the compiler expected a structure similar to `if(x>3){}`, but they received `if(x>3}`. However, if the first check returns false (first token needs to be `IF`), the compiler should not throw any errors, as there might be a variable declaration or a `for` loop instead of the `if`, and we have different rules implemented for them.

The question is: how can we know for sure which rule(s) to call? In the list of syntax rules (see first paragraph), there is a rule called `unit`, that is a combination of all possible statements and expressions to be found in AtomC. Checking for `unit`, until no more token is left, should be enough.

> **Note:** AtomC expects the entire logic to be stored in functions. No code outside functions and structure declarations is allowed, in comparison to other languages, such as C.

#### Implementation details
We usually need to keep track of the current token to be processed, either through a reference to it (in case of a linked list collection) or by storing the current index (in case of an array) as a state variable.
